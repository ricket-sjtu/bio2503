...
\runningheadrule
\firstpageheader{BIO2503}{Quiz 1}{March 10, 2025}
\runningheader{BIO2503}
...
\question[10]
IEEE表示单精度浮点数占据32位，分别用来表示符号位(1-bit)、阶码(8-bits)和尾数(23-bits)，其基本格式是：
...
其中$S$是符号位（0为正数，1为负数），$P$是阶码（移码，无符号数，偏移量为127），$M$是尾数（原码）。其数值的计算公式是：
$$
(-1)^{S} \times 2^{P-127} \times 1.M
$$

这里所谓的尾数就是二进制浮点数的小数部分，其整数位为1。那么，单精度数可以表示的数值范围是什么？其精度是什么？
...
\question[10]
请为乌有国国王出个主意：有128桶酒，其中有1桶是毒酒；48小时候要举行宴会，人喝了毒酒之后的11-12小时内将会死去；
国王决定用囚犯来试酒，而且不介意有多少囚犯会死。你能否用最少的囚犯来测试哪一桶才是毒酒。请问至少需要多少
...
\question[2]
n从1开始，每个操作可以选择对n加1或者对n加倍，若想获得整数2026，最少需要多少次操作？
\begin{choices}
	\choice 24
	\choice 21
	\choice 18
